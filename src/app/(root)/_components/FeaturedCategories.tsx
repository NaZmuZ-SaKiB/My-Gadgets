...
const FeaturedCategories = async () => {
  ...
  return (
    <section className="md:pt-6 pb-4">
      <h2 className="font-bold text-3xl text-gray-700">Featured Categories</h2>

      <div className="flex flex-wrap gap-3 md:gap-4 mt-8">
        {categories.map((category, i) => (
          <div
            key={`featured-category-${category._id}`}
            className={`rounded-xl flex flex-col justify-center items-center p-4 gap-4 flex-1 basis-32 sm:basis-40 hover:shadow-lg transition-shadow duration-500 cursor-pointer`}
